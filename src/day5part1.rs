...
impl IntcodeComputer {
    ...
    fn get_address_accounting_for_mode(&mut self, param_no: usize) -> usize {
        assert!(param_no < self.cur_param_count);
        match self.cur_param_modes.get(param_no).unwrap() {
            0 => self.mem[self.mem[self.cur_index + param_no] as usize] as usize,
            1 => self.mem[self.cur_index + param_no] as usize,
